...
namespace SqlManager.ViewModel
{
    class MakeDataVM : ValidationBase
    {
        ...
        string name { get; set; } = "";
        public MakeDataVM(MakeData.MakeData Iplugin, string _name)
        {
            plugin = Iplugin;
            name = _name;
        ...
        public SimpleCommand Search => new SimpleCommand()
        {
            ExecuteDelegate = x =>
            {
                GetAllUserTable(plugin.searchbox.Text);
            },
        ...
        private void GetAllUserTable(string tableName)
        {
            try
            {
        ...
        private string GetInsertSql()
        {
            ...
            foreach(var item in DataSource)
            {
                ...
                if(item.DATA_TYPE == "DATE")
                {
                    ...
                    else if(item.SelectedItem == "CONST")
                    {
                        var date = item.START_TIME ?? DateTime.Now;
                        temp += $@"TO_DATE('{date}', 'yyyy-mm-dd hh24:mi:ss')";
                    }
                    else
                    {
                        ...
                        {
                            date = item.START_TIME.Value.AddMinutes(start_seq++);
                        }
